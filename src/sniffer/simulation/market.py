...
#
# Три оси шума, вторую и третью добавил realcheck 03.09.2026. Первая — чужая
# КАТЕГОРИЯ, названная словом (велосипед, комната): её ловит `rank_items`
# разбором текста. Вторая — чужие МАРКА и КОРОБКА той же категории: на «ямаха»
# чат нёс Honda и Kymco, на «автомат» — механику. Структурная доска отсекла бы их
# полем (`_source_accepts` ниже так и делает), но у чата поля нет — отсев по
# тексту в `rank_items` единственная защита. Пока этих лотов в шуме не было,
# синтетика её и не мерила: `_source_accepts` вычищала чужую марку до отбора, и
# дыра жила незамеченной, пока её не вскрыл realcheck, а не симуляция.
#
# Третья ось — байк, названный ТОЛЬКО моделью, без слова категории
