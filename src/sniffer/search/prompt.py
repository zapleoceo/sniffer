...
которую прочтёт как «на рынке нет». Имя модели («Honda Vision», «Air Blade»)
исключением НЕ является, хотя это и настоящий текст объявления: поля модели у
такого источника нет, поэтому имя в query гасит те фильтры, что есть, а модель
отбирается уже в выдаче. Слова свойств для таких источников перечислены ниже
отдельным списком, и он короткий, потому что в нём только измеренные; чего там
нет — не пиши.
