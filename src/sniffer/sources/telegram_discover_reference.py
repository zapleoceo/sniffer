...
Числа ниже — решение владельца. Суточный потолок 1.09.2026 поднят с трёх до
десяти вступлений: очередь быстрее подключает уже проверенный стартовый набор,
но скользящее окно, пауза не меньше часа и стоп после FloodWait остаются
обязательными защитами единственного аккаунта.
...
# CLAUDE.md: не больше 10 вступлений в сутки. Окно скользящее, а не календарное,
# и это осознанно строже буквы: календарные сутки разрешают десять вступлений в
# 23:59 и ещё десять в 00:01, то есть двадцать за две минуты — ровно тот всплеск, от
...
# CLAUDE.md: `FloodWaitError` останавливает вступления «до конца суток». Буква
# правила оставляла лазейку: флуд в 23:59 держал шестьдесят секунд, и дальше
# работала только часовая пауза — замер дал два новых вступления в течение трёх
# часов после предупреждения. Поэтому стоп не короче полусуток НИКОГДА, а до
# конца суток — когда его остаток больше.
FLOOD_MIN_STOP = timedelta(hours=12)
...
# разбирается по десять чатов в скользящие сутки, и находка не имеет права оттеснить группу,
...
REJECT_ALREADY_INSIDE = "already_inside"
# Кандидат столько раз кончился неизвестным исходом, что дальше он не кандидат,
# а вечный потребитель суточных слотов: `reserve()` берёт наименьший приоритет,
# то есть его же, и очередь за ним не двигается.
REJECT_TOO_MANY_ATTEMPTS = "too_many_attempts"
...
class ChatCandidate:
    ...
    идут со 100. При десяти вступлениях в скользящие сутки очередь живёт днями, и порядок
...
class JoinState:
    ...
    воркера, одновременно прошедшие проверку, дадут одиннадцатое вступление.
    """
...
class CandidateQueue(Protocol):
    ...
    async def release(self, key: str) -> int:
        """Вернуть кандидата в очередь и ответить, сколько попыток он уже съел.

        Число возвращается, а не хранится у вызывающего, по той же причине, по
        которой лимиты живут в БД: попытки обязаны переживать перезапуск
        процесса. Счётчик в памяти обнулялся бы вместе с ним, а вечный кандидат
        именно так и жил — забирая суточные слоты целиком, бессрочно.
        """
        ...
...
class JoinLedger(Protocol):
    ...
    рабочей и молча разрешала бы десять вступлений после каждого перезапуска —
    ...
    временны́х ворота (стоп после флуда, десять вступлений в скользящие сутки,
    ...
    недосчитать одно вступление, чем сделать одиннадцатое.
    """
...
class TelegramJoiner(Protocol):
    ...
    Читающих методов четыре (`resolve_username`, `check_invite`,
    `search_contacts`, `history`), действий ровно два: `join_public` / `join_invite` и
