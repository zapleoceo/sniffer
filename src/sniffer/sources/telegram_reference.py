...
Насколько далеко эта граница достаёт — важно не преуменьшить и не преувеличить.
Всё, что типизировано как `TelegramReader`, mypy держит: обращение к
`send_message` у такого объекта не проходит проверку — `has no attribute`.
А вот `TelegramClient`, взятый прямо
из Telethon, для mypy — `Any`: библиотека без `py.typed`, при
`ignore_missing_imports` у неё «есть» любой метод, и ни mypy, ни ruff отправку
через такой объект не заметят. Поэтому запрет держится ДВУМЯ вещами: протоколом
в типах и тестом `test_adapter_never_calls_an_outgoing_method`, который ищет
исходящие вызовы регуляркой по всем модулям пути чтения.
"""
...
class TelegramReader(Protocol):
    ...
    (spec-v2, 6.1), а для сервиса на одном аккаунте это означает простой.

    Протокол закрывает путь через себя, но не обход мимо: у голого
    `TelegramClient` для mypy есть любой метод (см. докстринг модуля). Обход
    ловит тест по исходным текстам, а не проверка типов.
    """
