...
def blocking_question(passport: Passport, asked: Sequence[str]) -> Question | None:
    ...
    Всё прочее — город, бюджет, коробка, состояние, марка — до первой выдачи не
    спрашивается. passport.md требует показать выдачу рано и уточнять обратной
    связью на карточках: сказать «дорого», глядя на пять карточек, проще и
    умнее, чем назвать бюджет в пустоту.

    Воронка из трёх вопросов (категория→город→бюджет) этот принцип нарушала —
    живая жалоба владельца 04.09.2026: на «Я в Нечанге, нужен мотак» бот всё
    равно спрашивал бюджет, на «байк за 14 лямов» — город. Клиента допрашивали
    прежде, чем помочь. Теперь до поиска стоит один вопрос и только тот, без
    которого нечего искать, — что именно ищем.

    Город блокирующим НЕ считается: его подставляет `default_city` (Нячанг), а
    неизвестный/необслуживаемый ловит `is_served` в боте отдельным ответом.
    Бюджет уточняется обратной связью: «дорого» под карточками режет потолок, а
    если бюджет не назывался — там же и спрашивает.
    """
