...
# Сколько первых букв делают два слова одним и тем же словом (`_same_stem`).
# Падежное окончание просьбу не меняет: «нячанге» и «нячанг» — один город,
# «квартиру» и «квартира» — одна категория. Четыре буквы, а не две: с двумя
# «дом» сошёлся бы с «доллар», а с точным равенством телеграфное «скутер
# нячанг» перестало бы быть повтором «ищу скутер в нячанге».
_STEM_PREFIX = 4
...
def restates(current: Passport, fresh: Passport) -> bool:
    ...
    * **Слова** — приносит ли формулировка хоть одно новое слово. Разбор видит
      не всё: он читает намерение, категорию и город, а «ищу скутер, только
      подешевле» на всех трёх отвечает то же самое. Повторяют же обычно короче
      или телеграфно — «скутер в нячанге», «скутер нячанг», — и ни одного
      слова, которого не было, такой повтор не добавляет.
    """
...
def _adds_no_new_words(before: str, after: str) -> bool:
    ...
    Асимметрия нарочная: короче — повтор, длиннее — новый запрос. Дописанное
    слово и есть новое сведение, а копипаст собственной фразы — тот самый обход
    лимита, ради которого эта половина и нужна, — новых слов не приносит.
    ...
    без нормализации написаний лечить их пришлось бы догадками.
    """
    known, said = _words(before), _words(after)
...
def _same_stem(first: str, second: str) -> bool:
    """Одно слово в разных падежах: общая основа в `_STEM_PREFIX` букв.

    Слово короче основы совпадает только само с собой: у «в» и «до» вариантов
    написания нет, а сойтись по двум буквам они могли бы с чем угодно.
    """
    stem = first[:_STEM_PREFIX]
    return first == second or (len(stem) == _STEM_PREFIX and second.startswith(stem))
