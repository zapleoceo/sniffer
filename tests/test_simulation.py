...
@pytest.mark.xfail(
    strict=False,
    reason=(
        "Осталась НЕ категория: её живой поиск теперь отсекает, и мусор в выдаче "
        "упал с 38 карточек из 92 до 16 из 80 (замер 02.09.2026). Остаток — три "
        "разных дела, и ни одно не решается ещё одним фильтром «здесь и сейчас». "
        "Свойства (коробка, марка, цена выше бюджета) отбираются баллом, а не "
        "выбрасыванием, и это осознанный выбор: продавец не обязан называть "
        "свойство, а дороже бюджета — законная карточка, когда точных нет. "
        "Объём двигателя доска отбирает структурным полем, а у чата такого поля "
        "нет. И `honda-cb200x` проходит в пустоту между двумя фильтрами: своей "
        "модели у него нет в справочнике, а слова категории нет в тексте — "
        "«не смогли прочитать» это не «не подходит». Не strict: каждая из трёх "
        "причин закрывается своей задачей, и тест позеленеет сам, когда доедет "
        "последняя из них, — правки он не потребует"
    ),
)
